...
# exclude_features_filter = ["X_calc", "V_calc", "dXdt_calc", "dVdt_calc", "Xlag1_calc"] # once it is analyzed filter feature selection                                                                   
# vars_wrapper = [v for v in vars if v not in exclude_features_filter] # mu_calc, Plag1, t, t_ind, T can be filter by wrapper methods
# permanent_features = ["I", "P"] # cant be exclude (heatmap analysis)

# # --------- Wrapper ------------
# wrapper_feature_selection(df_global, X_vars = vars_wrapper, y_var=target[2*i], c_var = permanent_features, model_names_w=wrapper_models, 
#                     model_names_p=permutation_models, out_path=f"results/feature_selection/global/wrapper/{target[2*i]}")
# wrapper_feature_selection(df_global, X_vars = vars_wrapper, y_var=target[2*i + 1], c_var = permanent_features, model_names_w=wrapper_models, 
...
vars = [v for v in all_features if v not in exclude_features]
vars_ead = vars
compute_ead(df_global, vars = vars_ead, results_root="results/data_analysis/ead/P")

# # ----------- Feature selection -------------- 
# ----------- Filter methods --------------
vars_filter = [v for v in vars_ead if v not in ("P")]
filter_feature_selection(df_global, X_vars = vars_filter, y_var="P", out_dir=f"results/feature_selection/global/filter/P")

exclude_features_filter = ["X_calc", "V_calc", "dXdt_calc", "dVdt_calc", "Xlag1_calc"] # once it is analyzed filter feature selection                                                                   
vars_wrapper = [v for v in vars_filter if v not in exclude_features_filter] 
permanent_features = ["I"] # cant be exclude (heatmap analysis)

# --------- Wrapper & Embedded methods ------------
wrapper_feature_selection(df_global, X_vars = vars_wrapper, y_var="P", c_var = permanent_features, model_names_w=wrapper_models, 
